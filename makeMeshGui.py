...
handlesD = np.load('saves/HD_handlesList_1.npy')
existHandles = np.load('saves/HD_existHandles_1.npy')
exceptList = np.load('saves/HD_exceptionList_1.npy')

exceptInd = []
for i in range(len(exceptList)):
    if exceptList[i] == 1:
        exceptInd.append(i)
handlesExist = np.delete(handlesD, exceptInd, 0)
avghand = np.mean(handlesExist, axis=0)
avghand = avghand[:17]
handles = np.append(avghand, np.array([[[325, 245]]]), axis=0)
...
class Application(tkinter.Frame):
    def __init__(self, master=None):
        ...
        avghand = np.mean(handlesExist, axis=0)
        avghand = avghand[:17]
        self.handles = np.append(avghand, np.array([[[325, 245]]]), axis=0)
    ...
    def getCoordinate(self, event):
        ...
        self.handles = np.append(self.handles, np.array([[[event.x-2, event.y-2]]]), axis=0)
        print(self.handles.shape)
    ...
    def dispImg(self):
        ...
        for i in range(len(self.handles)):
            self.test_canvas.create_oval(self.handles[i][0][0]-self.pointRad, self.handles[i][0][1]-self.pointRad, self.handles[i][0][0]+self.pointRad, self.handles[i][0][1]+self.pointRad, fill="red", tag = "pointG")
    ...
    def save(self):
        np.save('saves/meshPoints__', self.handles)
        print("saved.")
